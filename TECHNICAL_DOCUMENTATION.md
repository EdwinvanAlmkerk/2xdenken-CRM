...
## Laatste update
- Datum: 2026-04-13
- Opmerking: Globale zoekbalk fors uitgebreid. `globalSearch` en `renderGlobalResults` in `js/utils.js` zijn herschreven met scoring (`_searchScore`), match-highlighting (`_highlightMatch`), groepering per type (max 5 per groep), agenda-zoekopdrachten op titel/locatie, en keyboard-navigatie (Ctrl/Cmd+K, Arrow, Enter, Escape) via nieuwe module-state `_globalSearchItems`/`_globalSearchIdx` en helpers `_updateGlobalSearchHighlight`/`_execGlobalResult`. `css/style.css` kreeg een uitgebreide SEARCH-sectie met stijlen voor group-labels, rijen, highlights, footer en een Ctrl+K-badge. `index.html` heeft een bredere search-wrap (360px), bijgewerkte placeholder en de extra `.gs-kbd-badge`. Geen schema- of data-flow wijzigingen buiten deze zoekfunctie.

...
## Laatste update
- Datum: 2026-04-13
- Opmerking: E-mailmodule uitgebreid met verwijderfunctionaliteit voor IMAP-mails. `supabase/functions/fetch-emails/index.ts` heeft nieuwe `ImapClient`-methodes `moveMessage`, `expungeMessage` en `findTrashFolder`, plus een `action=delete` serve-branch die op basis van folder naar Trash verplaatst (INBOX → gedetecteerde Trash-map) of permanent wist (`__trash__` → expunge). In `js/pages/email-page.js` zijn `deleteMail(uid)` met optimistische update en rollback en een nieuwe rode trash-knop in `renderEmailDetail` (voor zowel inbox als trash, met context-afhankelijke tooltip) toegevoegd. Geen database-wijzigingen; Edge Function opnieuw deployen met `--no-verify-jwt`.

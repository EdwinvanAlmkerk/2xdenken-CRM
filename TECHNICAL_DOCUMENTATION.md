...
## Laatste update
- Datum: 2026-04-13
- Opmerking: E-mailmodule uitgebreid met map "Verwijderde items". Frontend (`js/pages/email-page.js`) kreeg nieuwe state `_trashMessages`/`_trashLoading`/`_trashError`, een `fetchTrash()`-functie en ondersteuning voor `_emailFolder === 'verwijderd'` in `selectInboxEmail()` en `renderEmailPage()` (sidebar-entry, auto-load, loading/error/refresh en van-kolom). In de Edge Function `supabase/functions/fetch-emails/index.ts` is `ImapClient.select()` strict gemaakt (throwt bij NO/BAD) en is `ImapClient.selectTrash()` toegevoegd die kandidaat-mapnamen als `[Gmail]/Trash`, `[Gmail]/Prullenbak`, `Trash`, `Deleted Items` etc. probeert. De folder-parameter `__trash__` wordt server-side als alias herkend voor zowel lijst- als single-UID fetches. Edge function moet opnieuw gedeployed worden (`supabase functions deploy fetch-emails`). Geen schema-wijzigingen.

...
## Laatste update
- Datum: 2026-04-15
- Opmerking: De agenda-koppeling is in de UI hernoemd van "Outlook Agenda" naar het bronneutrale "Agenda (gepubliceerde feed)" (Instellingen-card, agenda-popup en CRUD-meldingen); de iCloud-stappen staan nu als primaire instructie, Outlook/Google blijven ondersteund. De onderliggende tabel (`outlook_settings`), Edge Function (`fetch-outlook-ics`) en state-variabelen (`_outlookEvents`, `DB.outlookSettings`) zijn ongewijzigd. Daarnaast is er een Excel-export toegevoegd op de agenda-pagina (knop in de toolbar, modal met datumbereik en optioneel externe events, `.xlsx` met totaalregel) via SheetJS, dat nu als extra CDN-library in `index.html` wordt geladen. Nieuw `download`-icoon in `js/utils.js`.
